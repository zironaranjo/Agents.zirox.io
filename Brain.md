...
- Integracion OpenRouter MVP:
  ...
  - Boardroom (departamentos): `POST /api/boardroom/draft` genera borradores con `OPENROUTER_API_KEY`. Modelo por defecto gratuito `google/gemma-2-9b-it:free` con reintento automatico en otros `:free` si el proveedor falla; opcional `OPENROUTER_BOARDROOM_MODEL`. Prompt segun `departmentSlug` (`lib/boardroom-draft.ts`).
